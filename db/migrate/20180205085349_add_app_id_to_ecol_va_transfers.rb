class AddAppIdToEcolVaTransfers < ActiveRecord::Migration
  def change
    add_column :ecol_va_transfers, :app_id, :string, :limit => 20, :comment => 'the identifier for the client to process a payment'    
    ...
    add_column :ecol_va_transfers, :rev_unblock_va_txn_id, :integer, comment: 'the id of ecol_va_txns table which is required to show the transaction detail'
